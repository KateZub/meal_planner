...
async def list_meal_plans(
    offset: int = 0,
    limit: int = 10,
    criterion: actions.MealPlanCriterionType = "name",
    ...
    Lists meal plans
    """
    result = await actions.list_meal_plans(db, offset=offset, limit=limit, criterion=criterion, direction=direction)
    return result


@router.post("/", status_code=201)
async def create_meal_plan(meal_plan_entry: NewMealPlanEntry, db: Connection = Depends(db.get_db)) -> MealPlan:
    """
    ...
    await common.save(db, meal_plan)
    await common.load(db, meal_plan)
    return meal_plan


@router.get("/{meal_plan_id}/")
async def load_meal_plan(meal_plan_id: int, db: Connection = Depends(db.get_db)) -> MealPlan:
    """
    Returns meal plan
    """
...
async def update_meal_plan(meal_plan_id: int, meal_plan_entry: MealPlanEntry, db: Connection = Depends(db.get_db)) -> MealPlan:
    """
    Updates meal plan
    """
    meal_plan = MealPlan(id=meal_plan_id)
    ...
    if entry_data.get("name"):
        try:
            await common.load(db, MealPlan(name=entry_data["name"]))
        except NotFoundException:
            pass
        else:
    ...
    await common.save(db, new_meal_plan)
    await common.load(db, new_meal_plan)
    return new_meal_plan


@router.put("/{meal_plan_id}/recipes", status_code=204)
async def add_recipes(meal_plan_id: int, recipes: list[MealPlanRecipe], db: Connection = Depends(db.get_db)) -> None:
    """
    Adds recipes to the meal plan. recipe_name or recipe_id must be given.
    """
    meal_plan = MealPlan(id=meal_plan_id)
    await common.load(db, meal_plan)
    await common.add_entity_items(db, meal_plan, recipes)


@router.delete("/{meal_plan_id}/recipes", status_code=204)
async def remove_recipes(meal_plan_id: int, recipes: list[int] | list[str], db: Connection = Depends(db.get_db)) -> None:
    """
    ...
    meal_plan = MealPlan(id=meal_plan_id)
    await common.load(db, meal_plan)
    await common.remove_entity_items(db, meal_plan, recipes)


@router.get("/{meal_plan_id}/shopping_list/")
async def get_shopping_list(meal_plan_id: int, db: Connection = Depends(db.get_db)) -> list[RecipeIngredient]:
    """
    Returns shopping list for the meal plan
    """
    meal_plan = MealPlan(id=meal_plan_id)
    result = await actions.generate_shopping_list(db, meal_plan)
    return result
